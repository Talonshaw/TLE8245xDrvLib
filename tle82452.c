...
void HWA_TLE82452_CheckFaultn(void)
{
    ...
    //VIO supply.
    faultnPin = drv_ReadPin(TLE_FAULTN_PORT, TLE_FAULTN_PAD);
    if (faultnPin == 0)
    {
...
void HWA_TLE82452_Handler_10ms(void)
{
    ...
    {
    case INIT_STEP_1: //Step.1  Apply the clock signal to the CLK pin, then transition the RESN pin from low to high.
        //CLK
        ...
        //	drv_ResetPin(TLE_RST_PORT, TLE_RST_PAD);
        //	DelayUs(100);
        drv_SetPin(TLE_RST_PORT, TLE_RST_PAD);
        tle82452_Init_Steps = INIT_STEP_2;
        break;
    case INIT_STEP_2: //Step.2	 Wait for the TPOR timer to elapse The SPI bus will begin responding after the TPOR has elapsed.
        ...
        {
            tle82452_Init_Steps = INIT_STEP_3;
        }
        break;
    case INIT_STEP_3: //Step.3	 Write to the CLK-DIVIDER register via the SPI interface. Enable the watchdog and set the system clock divider.
        ...
        tle82452_Init_Steps = INIT_STEP_4;
        break;
    case INIT_STEP_4: //Step.4  Wait for TWU wake up timer to elapse TWU has elapsed when the AZ bit returns to 1 in the SPI register.
        //TWU /ms = 65536/(FSYS/kHz) + 0.1
        //Wait Device Ready.
        if (drv_ReadPin(TLE_FAULTN_PORT, TLE_FAULTN_PAD) == 1)
        {
            //Set PWM reg
            ...
            drv_SetPin(TLE_EN_PORT, TLE_EN_PAD);
            tle82452_Init_Steps = INIT_READY; //Init end.
        }
        break;
    case INIT_READY:
